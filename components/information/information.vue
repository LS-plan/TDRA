		...
		<scroll-view class="data_body" scroll-y :style="{height:scrollHeight}">
			<template v-if="true">
				...
				<view class="friend_operate">
				  ...
				  <qiun-data-charts
					...
					<view class="charts-box">
						<qiun-data-charts type="radar" :chartData="analyseData" background="none"></qiun-data-charts>
					</view>
				</view>
				<view class="split_line"></view>

				<view class="friend_operate" @click="xueshi()">
					<view class="title">综合评价</view>
					<text class="conclusion">
						经认定，该同学在校期间累计获得：
						思想成长类课程学分 {{ circleData[0].series[0]['value']  }} 个，
						创新创业类课程学分{{ circleData[2].series[0]['value'] }}个，
						文体发展类课程学分{{ circleData[1].series[0]['value'] }}个，
						社会实践与志愿服务类课程学分{{ circleData[3].series[0]['value'] }}个。
					</text>
					<button size="25%" type="primary" @click="xueshi()">点击查看学时预警情况</button>
					<button size="25%" type="default">{{ zhuangtai }}</button>
					<text>\n \n</text>
				</view>
			...
			<template v-else>
				<view class="container padding_stand-big normal_color">
					<li class="iconfont icon-cry cry"></li>
				</view>
			</template>
		</scroll-view>
	</view>
...
<script>
	...
	// import title from "@/pages/student-information/student-information.vue"
	...
	import analyseData from '../../static/json/information/data1.json';
	...
	export default {
		...
		methods: {
			...
			xueshi(){
				...
				if(this.circleData[0].series[0]['value'] >=2 && this.circleData[1].series[0]['value'] >=2 && this.circleData[2].series[0]['value'] >=2 &&this.circleData[3].series[0]['value'] >=2){
					this.zhuangtai="学时满足要求"
